...
道路(`DEFAULT_ROADS`): 東西幹線 4 本(幅 6m、2 車線、`widthType=3`)、南北区画道路 4 本(幅 4.5m、1 車線、`widthType=2`)、D ブロック前の路地 1 本(幅 3m、1 車線、`widthType=2`)。全道路に `width`・`numberOfLanes`・`widthType` を入れてあるので、退避経路のテストでは属性を落として使う(§6 の検証で幾何推定 3.38m ← 実幅 4.0m を確認済み)。
