...
### 6.6 戸田市 2022 年度(v4)で確認できたこと(2026-09-04)

- G空間情報センターの CKAN API(`package_search?q=plateau`、495 件)から `plateau-11224-toda-shi-2022` の CityGML v4 zip(483MB)を取得。展開後 11GB(DEM が 9.1GB)。
- `bldg:yearOfConstruction` は全棟に要素があるが、約 1/3 は「0001」等の番兵値。**1800 年未満を未知として扱う**ようパーサを修正した。住宅棟の充足率 74.6%。
- `bldg:storeysAboveGround`・`bldg:usage` は 100%。住所(xAL)は無し。浸水想定は `udx/fld` 側(建物属性としては未確認)。
- `tran:Road` は 9,293 本、`tran:function` のみ(codelist の値は 1/2/3/9020)。`uro:RoadStructureAttribute` は無い → §2.4 の退避 3(道路面の弦長)が主経路になる。`RoadStructureAttribute_widthType.xml` も同梱されていない。
- `udx/dem` は 2 次メッシュ単位で 1 ファイル 600MB。fast-xml-parser ではメモリに載らないので、TIN をストリーミングで読む実装が要る(未着手)。
- 築年クラスタは戸建て相当(用途 411/413・底面 35〜220 m²・高さ 13 m 以下)に絞って 1,840。2010〜2016 年の街区 132、最大 32 軒(2012〜2013 年分譲)。
