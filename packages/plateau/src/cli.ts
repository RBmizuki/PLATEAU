...
if (cmd === 'ingest') {
  ...
  const demDir = arg('--dem');
  let demApplied = 0;
  if (demDir) {
    const tris = [];
    for (const file of listGml(demDir)) tris.push(...parseDemTriangles(readFileSync(file, 'utf8')));
    if (tris.length > 0) {
      demApplied = applyDem(buildings, new DemSampler(tris));
      console.error(`dem: ${tris.length} triangles, elevation/slope written to ${demApplied}/${buildings.length} buildings`);
    }
  }
